...
//
	...
	void tts_speak(const char* text) {
		...
		NSString *stringToBeSpoken= [NSString stringWithUTF8String:text];
			...
			utterance.rate = (tts_rate+0.5) * AVSpeechUtteranceDefaultSpeechRate;	// within the range of AVSpeechUtteranceDefaultSpeechRate*0.5 to AVSpeechUtteranceDefaultSpeechRate*1.5
	...
	bool tts_setRate(float rate) {
		...
		if ( tts_rate > 1.0f ) tts_rate = 1.0f;
		#if defined(DM_PLATFORM_OSX)	
			// NSSpeechSynthesizer supports words per minute,
			// Human speech is 180 to 220.
			// Let 200 be the default.
			[pEngine->speechSynthesizer setObject:[NSNumber numberWithInt:100+tts_rate * 200]
